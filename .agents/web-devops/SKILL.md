...
- Dependency scanning: `npm audit`, `pip-audit`, Dependabot / Renovate; integrate Socket, Snyk, or Aikido for PR-level supply chain monitoring
- **Supply chain attack defense:** lifecycle scripts (`postinstall`/`preinstall`) execute arbitrary code on `npm install` — use pnpm v10+ (blocks scripts by default) with an `allowBuilds` allowlist, or `ignore-scripts=true` in `.npmrc` for npm projects; set `minimumReleaseAge: 1440` (1-day cooldown); always commit the lockfile and install with `--frozen-lockfile` in CI
...
- `references/security.md` — full application security: supply chain attack defense (Shai-Hulud, lifecycle script blocking, pnpm allowBuilds, minimumReleaseAge, lockfile discipline, Socket/Snyk), password hashing, JWT/refresh tokens, httpOnly cookies, session management, RBAC patterns, rate limiting (HTTP + WebSocket), IP controls, WAF, generic error handling, backup strategy, pre-launch checklist
