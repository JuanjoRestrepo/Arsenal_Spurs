...
El modelo va más allá de los datos históricos brutos. Para cada equipo en cada competición, se aplican multiplicadores calibrados sobre las tasas esperadas de gol (λ y μ del modelo Dixon-Coles) basados en:
...
- Tridente Dembélé/Barcola/Ramos en forma → escala ataque 0.989 (excelente)
...
FBref (Ligue 1)         ─┤─→ [Pandera Validation] → [Combined Dataset: 657 matches]
                ...
                [Time-Decay Hyperparameter Tuning]
                  Grid Search: α ∈ {0.001, 0.003, 0.0065, 0.010, 0.015}
                  Optimal α = 0.0100 (half-life ≈ 69 días)
                          │
                          ↓
                [Dixon-Coles Model Fit — Multi-Liga]
                  Parameters: attack_i, defense_i, home_adv, ρ
                  Constraint: Σ attack_i = 0 (identifiability)
                          │
                          ↓
              [Contextual Adjustment Engine]
                  TeamContext: attack_scale × defense_scale × exp(motivation)
...
### Modelo Matemático — Dixon-Coles (1997)
...
- $w_k = e^{-\alpha^* \cdot t_k}$ — peso de decaimiento temporal (α* = 0.0100)
- $\tau_\rho$ — corrección de Dixon-Coles para marcadores bajos (0-0, 1-0, 0-1, 1-1)
...
| `images/executive_summary.png` | Dashboard ejecutivo oscuro con 4 KPIs |
...
*Modelo entrenado con datos hasta 15 de Mayo de 2026. Producción: Arsenal & Tottenham Predictive Intelligence Platform v2.0.*
